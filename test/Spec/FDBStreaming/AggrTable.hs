...
module Spec.FDBStreaming.AggrTable
  ( tableProps,
  )
where
...
import Statistics.Monoid (CalcMean, MeanKBN, StatMonoid (..))
...
propTableRange ::
  forall v.
  (TableSemigroup v, RangeAccessibleTable v, Eq v) =>
  Subspace ->
  Database ->
  ByteString ->
  v ->
...
nonAtomicSemigroupProps testSS db =
  testGroup
    "non-atomic table helper functions"
